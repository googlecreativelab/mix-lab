...
export class JamComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.eventsService.broadcast('onJamPage');
    ...
    this.eventsService.on('playBtnClick', () => {
      this.canShowFallback = true;
    });
    ...
    if (this.isTouch) {
      ...
      this.eventsService.on('jamStart', this.onJamStart.bind(this));
    } else {
  ...
  ngOnDestroy() {
    this.clearIntroTimeouts();
    this.eventsService.broadcast('offJamPage');
    ...
    this.eventsService.off('jamStart', this.onJamStart.bind(this));
  }
